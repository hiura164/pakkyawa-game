...
(() => {
  ...
  function loadSave() {
    try {
      return Object.assign({ unlocked: 1, best: {}, coins: 0 }, JSON.parse(localStorage.getItem(SAVE_KEY) || '{}'));
    } catch (_) {
      return { unlocked: 1, best: {}, coins: 0 };
    }
  }
  ...
  function update(dt) {
    ...
    if (input.justJump && player.ground) { player.vy = -690; player.ground = false; }
    ...
    for (const c of coins) if (!c.got && Math.hypot(player.x + player.w / 2 - c.x, player.y + player.h / 2 - c.y) < c.r + 28) { c.got = true; score++; }
    const pr = { x: player.x, y: player.y, w: player.w, h: player.h };
    for (const e of enemies) if (!e.dead && rect(pr, e)) { const stomp = prevBottom <= e.y + 12 && player.y + player.h >= e.y && player.vy >= 0; if (stomp) { e.dead = true; score += 5; player.vy = -500; } else loseLife(); }
    if (boss.active && !boss.dead && rect(pr, boss)) { const stomp = prevBottom <= boss.y + 15 && player.y + player.h >= boss.y && player.vy >= 0; if (stomp && boss.inv <= 0) { boss.hp--; boss.inv = .45; score += 10; player.vy = -540; if (boss.hp <= 0) boss.dead = true; } else if (!stomp && boss.inv <= 0) loseLife(); }
